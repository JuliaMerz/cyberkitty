...
from server.models import Query, ApiCall
...
def query_executor(system_prompt, prompt, user, previous_messages=None):
    """
    ...
    query = Query(user=user, prompt=prompt, system_prompt=system_prompt, previous_messages=previous_messages, response=complete_output)
    ...
    while True:
        try:
            ...
            call_cost = None
            if response.usage:
                call_cost = calc_cost(response.usage.prompt_tokens, response.usage.completion_tokens)

            api_call = ApiCall(query=query, success=True, cost=cost, input_messages=messages, output=choice.message)
