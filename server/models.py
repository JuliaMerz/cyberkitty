...
class StoryOutline(Base):
    ...
    outline_mainevents_raw = db.Column(db.Text, nullable=True, default=None)
    editing_notes = db.Column(db.Text, nullable=True, default=None)
    outline_mainevents_improved = db.Column(
...
class ChapterOutline(Base):
    ...
    previous_chapter = db.Column(db.ForeignKey(
        'chapter_outlines.id'), nullable=True)
    ...
    # GPT GENERATED
    # setting = db.Column(db.Text, nullable=True, default=None)
    # Structure:  for each scene, events, primary function, secondary function
...
class SceneOutline(Base):
    ...
    previous_scene = db.Column(db.ForeignKey(
        'scene_outlines.id'), nullable=True)
    ...
    summary = db.Column(db.Text)  # pulled from chapter outline
    context = db.Column(db.Text)  # pulled from chapter outline
...
class ApiCall(Base):
    ...
    # null for when the openai doesn't return usage numbers
...
story_query = db.Table('story_query',
                       ...
                       db.Column('query_id', db.Integer, db.ForeignKey(
                           'query.id'), primary_key=True)
                       )

story_outline_query = db.Table('story_outline_query',
                               Base.metadata,
                               db.Column('story_outline_id', db.Integer, db.ForeignKey(
                                   'story_outline.id'), primary_key=True),
                               db.Column('query_id', db.Integer, db.ForeignKey(
                                   'query.id'), primary_key=True)
                               )
...
chapter_outline_query = db.Table('chapter_outline_query',
                                 Base.metadata,
                                 db.Column('chapter_outline_id', db.Integer, db.ForeignKey(
                                     'chapter_outline.id'), primary_key=True),
                                 db.Column('query_id', db.Integer, db.ForeignKey(
                                     'query.id'), primary_key=True)
                                 )
...
scene_outline_query = db.Table('scene_outline_query',
                               Base.metadata,
                               db.Column('scene_outline_id', db.Integer, db.ForeignKey(
                                   'scene_outline.id'), primary_key=True),
                               db.Column('query_id', db.Integer, db.ForeignKey(
                                   'query.id'), primary_key=True)
                               )
...
scene_query = db.Table('scene_query',
                       Base.metadata,
                       db.Column('scene_id', db.Integer, db.ForeignKey(
                           'scene.id'), primary_key=True),
                       db.Column('query_id', db.Integer, db.ForeignKey(
                           'query.id'), primary_key=True)
                       )
