/**
 * TAKEN FROM: https://github.com/rexxars/eventsource-parser/blob/main/src/parse.ts
 *
 * Modified to emit events for partial completions, allowing us to stream partial chunks
 *
 ...
 */
import type {EventSourceParseCallback, EventSourceParser} from './eventTypes';
...
function hasBomArray(buffer: Uint8Array) {
  return BOM.every((charCode, index) => buffer[index] === charCode);
}

function hasBom(buffer: string | Uint8Array) {
  if (typeof buffer === 'object') {
    return hasBomArray(buffer as Uint8Array)
  }

  return BOM.every((charCode: number, index: number) => (buffer as string).charCodeAt(index) === charCode)
}
